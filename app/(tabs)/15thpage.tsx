...
export default function Task3complete({ route }) {
  const { enttext1, enttext2 , enttext3 ,tasks1, tasks2, tasks3, tasks4 } = route.params;
  const navigation = useNavigation();

  return (
    <ScrollView  style={{backgroundColor: 'lightblue'}} contentContainerStyle={styles.container}>
      <Text style={styles.title}>Great you Completed these TASKS: {'\n'}  {'\t'} {enttext1} , {enttext2} , {enttext3} </Text>
      
      <Button
        title="Proceed To the NEXT TASK"
        onPress={() => 
          navigation.navigate('Task4', { tasks1, tasks2, tasks3, tasks4, enttext1 , enttext2 , enttext3 })
        }
        color="#6200ea"
      />
      ...
      <Button
        ...
        onPress={() => 
          navigation.navigate('alltasks', { enttext1, enttext2 , enttext3, tasks1, tasks2, tasks3, tasks4 })
        }
        color="#d32f2f"
      />
