...
export default function Task1complete({ route }) {
  const { enttext1, tasks1, tasks2, tasks3, tasks4 } = route.params;
  const navigation = useNavigation();

  return (
    <ScrollView  style={{backgroundColor: 'lightblue'}} contentContainerStyle={styles.container}>
      <Text style={styles.title}>Great! You Completed This Task: {enttext1}</Text>
      
      <Button
        title="Proceed To the NEXT TASK"
        onPress={() => 
          navigation.navigate('Task2', { tasks1, tasks2, tasks3, tasks4, enttext1 })
        }
        color="#6200ea"
      />
      ...
      <Button
        ...
        onPress={() => 
          navigation.navigate('alltasks', { enttext1, tasks1, tasks2, tasks3, tasks4 })
        }
        color="#d32f2f"
      />
